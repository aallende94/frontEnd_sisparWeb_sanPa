...
@Component({
  ...
  templateUrl: './reporte-confirmadas.component.html',
  styleUrls: ['./reporte-confirmadas.component.scss']
})
export class ReporteConfirmadasComponent implements OnInit {
  ...
  submit() {
   ...
   buscartabla(){
    ...
    this.title = 'Visitas Confirmadas';

    this.serviReportVisitas.getReportVisitasConfirm().subscribe(
      data => {

        let result : any = data;
    
       let dataVisitasConfirm = result.dataVisitasConfirm;

        if (dataVisitasConfirm) {

         this.mostrarMensaje_err = false;
         this.listVisitasConfirm = dataVisitasConfirm
   ...
   export(form: any) {
    ...
    let arrayDatos = [];
        ...
        arrayDatos = this.listVisitasConfirm.map(
          x => ({
            Ci:              x.vis_ci,
            Nombre:          x.vis_nombre,
